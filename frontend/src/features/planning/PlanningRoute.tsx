...
export function PlanningRoute() {
  ...
  return <OrganizationLayout organizationId={organizationId}><nav aria-label="Breadcrumb"><ol className="civitas-cluster">{crumbs.map((crumb, i) => <li key={`${crumb.labelKey}-${i}`}>{crumb.href ? <Link to={crumb.href}>{crumb.labelKey}</Link> : <span aria-current="page">{crumb.labelKey}</span>}</li>)}</ol></nav>{state !== "ready" ? <PlanningAccessFallback state={state} headingRef={heading} /> : <ModuleUiErrorBoundary moduleId="planning" screenId={resolved!.routeId}><PlanningRemoteScreen organizationId={organizationId} screen={resolved!.screen} planId={resolved!.planId} availability="available" readOnly={false} access="allowed" /></ModuleUiErrorBoundary>}</OrganizationLayout>;
}
