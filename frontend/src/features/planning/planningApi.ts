...
export type PlanningUnitOrder = { roadmapId: string; version: number; etag: string; units: Array<{ id: string; rank: string }> };
...
export const usePlanningApi = () => { const { organizationApiFetch } = useApi(); return useMemo(() => ({
  ...
  reorderUnits: async (organizationId: string, roadmapId: string, orderedIds: string[], etag: string, signal?: AbortSignal) => { try { return await organizationApiFetch(organizationId, `${root(organizationId)}/roadmaps/${encodeURIComponent(roadmapId)}/units/order`, { method: "PUT", body: JSON.stringify({ orderedIds }), headers: { "If-Match": etag, "Idempotency-Key": crypto.randomUUID() }, signal }) as PlanningUnitOrder; } catch (e) { mapError(e); } },
  validateBlueprint: async (organizationId: string, blueprintId: string, signal?: AbortSignal) => { try { return await organizationApiFetch(organizationId, `${root(organizationId)}/assessment-blueprints/${encodeURIComponent(blueprintId)}/validation-runs`, { method: "POST", body: "{}", headers: { "Idempotency-Key": crypto.randomUUID() }, signal }); } catch (e) { mapError(e); } },
