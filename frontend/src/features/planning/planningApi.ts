...
export const usePlanningApi = () => { const { organizationApiFetch } = useApi(); return useMemo(() => ({
  ...
  getPlan: async (organizationId: string, planId: string, signal?: AbortSignal) => { try { return assertPlan(await organizationApiFetch(organizationId, `${root(organizationId)}/plans/${encodeURIComponent(planId)}`, { signal })); } catch (e) { return mapError(e); } },
