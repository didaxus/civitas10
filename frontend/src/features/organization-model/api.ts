...
const endpoint = (organizationId: string, suffix: string) => `/api/v1/o/${encodeURIComponent(organizationId)}/organization-model/${suffix}`;
export const useOrganizationModelApi = () => {
  const { organizationApiFetch } = useApi();
  return useMemo(() => {
    const get = <T,>(organizationId: string, suffix: string, signal?: AbortSignal) => organizationApiFetch(organizationId, endpoint(organizationId, suffix), { signal }) as Promise<T>;
    const send = <T,>(organizationId: string, suffix: string, method: "POST" | "PUT" | "DELETE", body: unknown, signal?: AbortSignal, headers: Record<string,string> = {}) => organizationApiFetch(organizationId, endpoint(organizationId, suffix), { method, signal, headers, body: JSON.stringify(body) }) as Promise<T>;
    ...
    return {
      ...
      readSensitiveEvidence: (evaluationId:string,reason:string)=>operation("organizationModel.readSensitiveMappingEvidence",(organizationId,signal)=>organizationApiFetch(organizationId,endpoint(organizationId,`evaluations/${encodeURIComponent(evaluationId)}/evidence`),{signal,headers:{"X-Access-Reason":reason}}) as Promise<Record<string,unknown>>),
