...
export const useOrganizationModelApi = (explicitSurface?:OrganizationModelSurface) => {
  ...
  return useMemo(() => {
    ...
    return {
      ...
      initializeOrganizationModel: { actionId:"organizationModel.editDraft" as const, mutate:(organizationId:string,idempotencyKey:string,signal?:AbortSignal)=>send<Record<string,unknown>>(organizationId,"drafts","POST",{},signal,{"Idempotency-Key":idempotencyKey}) },
