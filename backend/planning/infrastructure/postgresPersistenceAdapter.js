...
function decodeCursor(cursor) {
  ...
  try {
    ...
    if (typeof value.id !== "string" || Number.isNaN(Date.parse(value.updatedAt))) throw new Error();
    return value;
  } catch { throw new TypeError("Invalid planning cursor"); }
}
  ...
  function bind(db) {
    const persistencePort = {
      ...
      async createPlan(input) {
        ...
        const aggregate = Planning.create({ organizationId: input.organizationId, id: input.planId || input.id, profileId, name: input.name ?? input.title, content: input.content ?? input.description ?? input.payload ?? {}, actorId: input.actorId || "system" });
      ...
      async listPlans({ organizationId, constraints = {} }) {
        const limit = Math.min(Math.max(Number(constraints.limit) || 50, 1), 100);
        const cursor = decodeCursor(constraints.cursor);
        const values = [organizationId, limit + 1];
        let seek = "";
        if (cursor) { values.push(cursor.updatedAt, cursor.id); seek = `and (updated_at,id) < ($3::timestamptz,$4)`; }
        const result = await db.query(`select * from planning_plans where organization_id=$1 ${constraints.includeArchived ? "" : "and state <> 'archived'"} ${seek} order by updated_at desc,id desc limit $2`, values);
        const rows = result.rows.slice(0, limit);
        return { items: rows.map((row) => ({ organizationId: row.organization_id, planId: row.id, profileId: row.profile_id, name: row.name, status: row.state, version: row.revision, updatedAt: row.updated_at })), page: { cursor: constraints.cursor || null, nextCursor: result.rows.length > limit ? encodeCursor(rows.at(-1)) : null, limit } };
      },
      async updatePlan(input) {
        ...
        aggregate.revise({ name: input.name ?? input.title, content: input.content ?? input.description ?? input.payload ?? aggregate.toSnapshot().versions.find((v) => v.version === aggregate.currentVersion).content, actorId: input.actorId || "system" });
      ...
      async replaceProfile(input) {
        ...
        const configuration = { ...(input.configuration ?? input.profile ?? {}),
          ...(input.planningMode !== undefined ? { planningMode: input.planningMode } : {}),
        ...
        const result = await db.query(`insert into planning_profiles(organization_id,id,configuration) values($1,$2,$3)
          ...
          where planning_profiles.version=$4 returning *`, [input.organizationId, id, configuration, Number(input.ifMatch)]);
    ...
    const idempotencyLedgerPort = {
      lookup: async ({ organizationId, principalId, clientId, operationId, key }) => { if (!key) return null; const result = await db.query("select * from planning_idempotency where organization_id=$1 and principal_id is not distinct from $2 and authenticated_client_id is not distinct from $3 and operation_id=$4 and idempotency_key=$5", [organizationId, principalId || null, clientId || null, operationId, key]); return result.rowCount ? { fingerprint: result.rows[0].fingerprint, result: result.rows[0].result } : null; },
      recordSuccess: async ({ organizationId, principalId, clientId, operationId, key, fingerprint, result }) => db.query("insert into planning_idempotency(organization_id,principal_id,authenticated_client_id,operation_id,idempotency_key,fingerprint,result) values($1,$2,$3,$4,$5,$6,$7)", [organizationId, principalId || null, clientId || null, operationId, key, fingerprint || digest(result), result]),
    };
...
function profileDto(row) { return { organizationId: row.organization_id, profileId: row.id, configuration: row.configuration, planningMode: row.configuration?.planningMode, preferences: row.configuration?.preferences, version: row.version, updatedAt: row.updated_at }; }
