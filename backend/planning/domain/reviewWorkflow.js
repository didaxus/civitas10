...
function applyReviewEvent(state, event) {
  ...
  if (event.type === REVIEW_EVENTS.ASSIGNED) next.assignments.set(event.assignmentId, { assignmentId:event.assignmentId, reviewerId:event.reviewerId, planVersion:event.planVersion, active:true });
  ...
  if (event.type === REVIEW_EVENTS.APPROVED || event.type === REVIEW_EVENTS.REJECTED) {
    next.status = event.type === REVIEW_EVENTS.APPROVED ? REVIEW_STATUS.APPROVED : REVIEW_STATUS.REJECTED;
...
function assertAssignment(state, assignmentId, actorId, planVersion) {
  const a = state.assignments.get(assignmentId);
  if (!a || !a.active || a.reviewerId !== actorId) throw new ReviewWorkflowError('assignment_not_active');
...
function decide(state, input, decision, type) { assertAssignment(state, input.assignmentId, input.actorId, input.planVersion); return evolve(state, type, { ...input, decision }); }
const workflow = Object.freeze({
  assign(state, input) { if (state.status !== REVIEW_STATUS.DRAFT && state.status !== REVIEW_STATUS.REJECTED) throw new ReviewWorkflowError('plan_not_assignable'); return evolve(state, REVIEW_EVENTS.ASSIGNED, input); },
  submit(state, input) { if (state.status !== REVIEW_STATUS.DRAFT && state.status !== REVIEW_STATUS.REJECTED) throw new ReviewWorkflowError('plan_not_submittable'); if (![...state.assignments.values()].some((a)=>a.active && String(a.planVersion)===String(input.planVersion))) throw new ReviewWorkflowError('active_assignment_required'); return evolve(state, REVIEW_EVENTS.SUBMITTED, input); },
  approve(state, input) { if (input.actorId === state.authorId) throw new ReviewWorkflowError('self_approval_denied'); if (state.status !== REVIEW_STATUS.IN_REVIEW) throw new ReviewWorkflowError('plan_not_in_review'); return decide(state, input, 'approved', REVIEW_EVENTS.APPROVED); },
  reject(state, input) { if (state.status !== REVIEW_STATUS.IN_REVIEW) throw new ReviewWorkflowError('plan_not_in_review'); return decide(state, input, 'rejected', REVIEW_EVENTS.REJECTED); },
  draftFromApproved(state, input) { if (state.status !== REVIEW_STATUS.APPROVED) throw new ReviewWorkflowError('plan_not_approved'); return evolve(state, REVIEW_EVENTS.DRAFT_STARTED, { ...input, sourceApprovedVersion:state.approvedVersion, baseSnapshot:state.approvedSnapshot }); },
});
