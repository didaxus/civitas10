...
function fingerprint(input) { return createHash('sha256').update(stableJson(input)).digest('hex'); }
...
async function enforceIdempotency({ ports, context, request, responseFactory }) {
  ...
  const principalId = context.subjectId || context.clientId;
  const operationId = context.operation?.operationId;
  if (!nonEmpty(principalId) || !nonEmpty(operationId)) return { replay: toFailed(REMOTE_PROBLEM_CODES.AUTH_CONTEXT, PROBLEM_CATEGORIES.AUTHORIZATION, { correlationId: context.correlationId, detailKey: 'idempotency_scope_incomplete' }) };
  const scope = { organizationId: context.organizationId, principalId, operationId, key };
...
async function recordSideEffects({ tx, ports, context, event, audit, outbox, idem }) {
  ...
  if (idem?.key) await (target.idempotencyLedgerPort || ports.idempotencyLedgerPort).recordSuccess({ organizationId: context.organizationId, principalId: idem.principalId, operationId: idem.operationId, key: idem.key, fingerprint: idem.requestFingerprint, result: event.result, correlationId: context.correlationId });
}
function createPlanningApplicationServices(ports, options = {}) {
  ...
  async function createPlan(command, context) {
    ...
    const result = await commitAtomically(ports, async (tx) => {
      const saved = await (tx.persistencePort || ports.persistencePort).createPlan({ ...command, organizationId: context.organizationId, actorId: context.subjectId || context.clientId, status: command.status || PLAN_STATUSES.DRAFT, updatedAt: nowIso(clock) });
      const dto = planDto(saved);
      await recordSideEffects({ tx, ports, context, event: { result: dto }, audit: { action: 'planning.plan.created.v1', organizationId: context.organizationId, actorId: context.subjectId, targetId: dto.id, correlationId: context.correlationId }, outbox: { type: 'planning.plan.created.v1', organizationId: context.organizationId, aggregateId: dto.id, payload: dto, correlationId: context.correlationId }, idem });
      return dto;
    });
  ...
  async function listPlans(query, context) {
    ...
    return ok(pageDto(await ports.persistencePort.listPlans({ organizationId: context.organizationId, constraints: normalizeList(query) })), metaFromContext(context));
  }
  async function readPlan(query, context) {
    const ctx = validateContext(context, 'readPlan'); if (ctx) return ctx;
    const scoped = await validateTenantScope({ ports, context, resource: { organizationId: context.organizationId, planId: query.planId }, operation: NAMED_USE_CASES.readPlan.operationId }); if (scoped) return scoped;
    ...
    const tenant = await validateTenantScope({ ports, context, resource: found, operation: NAMED_USE_CASES.readPlan.operationId }); if (tenant) return tenant;
  ...
  async function updatePlan(command, context) {
    ...
    if (!expected) return toFailed(REMOTE_PROBLEM_CODES.PRECONDITION_REQUIRED, PROBLEM_CATEGORIES.PRECONDITION, { correlationId: context.correlationId, currentVersion: current.version || current.etag });
    ...
    const result = await commitAtomically(ports, async (tx) => {
      ...
      const saved = await (tx.persistencePort || ports.persistencePort).updatePlan({ ...command, organizationId: context.organizationId, actorId: context.subjectId || context.clientId, ifMatch: expected, updatedAt: nowIso(clock) });
      const dto = planDto(saved);
      await recordSideEffects({ tx, ports, context, event: { result: dto }, audit: { action: 'planning.plan.updated.v1', organizationId: context.organizationId, actorId: context.subjectId, targetId: dto.id, correlationId: context.correlationId }, outbox: { type: 'planning.plan.updated.v1', organizationId: context.organizationId, aggregateId: dto.id, payload: dto, correlationId: context.correlationId }, idem });
      return dto;
    });
    return ok(result, metaFromContext(context));
  }
  async function readProfile(query, context) { const ctx = validateContext(context, 'readProfile'); if (ctx) return ctx; const scoped = await validateTenantScope({ ports, context, operation: NAMED_USE_CASES.readProfile.operationId }); if (scoped) return scoped; const found = await ports.persistencePort.readProfile({ organizationId: context.organizationId }); if (!found) return toFailed(REMOTE_PROBLEM_CODES.NOT_FOUND, PROBLEM_CATEGORIES.NOT_FOUND, { correlationId: context.correlationId }); return ok(profileDto(found), metaFromContext(context)); }
  async function replaceProfile(command, context) {
    ...
    const idem = await enforceIdempotency({ ports, context, request: command, responseFactory: (r) => ok(profileDto(r), metaFromContext(context)) }); if (idem.replay) return idem.replay;
    const current = await ports.persistencePort.readProfile({ organizationId: context.organizationId });
    const expected = context.concurrency?.etag || context.concurrency?.expectedVersion || command.ifMatch;
    if (current && !expected) return toFailed(REMOTE_PROBLEM_CODES.PRECONDITION_REQUIRED, PROBLEM_CATEGORIES.PRECONDITION, { correlationId: context.correlationId, currentVersion: current.version || current.etag });
    if (current && String(current.version || current.etag) !== String(expected)) return toFailed(REMOTE_PROBLEM_CODES.PRECONDITION, PROBLEM_CATEGORIES.PRECONDITION, { correlationId: context.correlationId, expectedVersion: expected, currentVersion: current.version || current.etag });
    const result = await commitAtomically(ports, async (tx) => { if (current) await (tx.concurrencyPort || ports.concurrencyPort).assertIfMatch({ organizationId: context.organizationId, aggregateType: 'planning.profile', aggregateId: context.organizationId, ifMatch: expected, currentVersion: current.version || current.etag }); const saved = await (tx.persistencePort || ports.persistencePort).replaceProfile({ ...command, organizationId: context.organizationId, actorId: context.subjectId || context.clientId, ifMatch: expected, updatedAt: nowIso(clock) }); const dto = profileDto(saved); await recordSideEffects({ tx, ports, context, event: { result: dto }, audit: { action: 'planning.profile.updated.v1', organizationId: context.organizationId, actorId: context.subjectId, correlationId: context.correlationId }, outbox: { type: 'planning.profile.updated.v1', organizationId: context.organizationId, aggregateId: context.organizationId, payload: dto, correlationId: context.correlationId }, idem }); return dto; });
    return ok(result, metaFromContext(context));
  }
