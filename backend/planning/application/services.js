...
function createPlanningApplicationServices(ports, options = {}) {
  ...
  async function createPlan(command, context) {
    ...
    const result = await commitAtomically(ports, async (tx) => {
      ...
      await recordSideEffects({ tx, ports, context, event: { result: dto }, audit: { action: 'planning.plan.created', organizationId: context.organizationId, actorId: context.subjectId, targetId: dto.planId, correlationId: context.correlationId }, outbox: { type: 'planning.plan.created', organizationId: context.organizationId, aggregateId: dto.planId, payload: dto, correlationId: context.correlationId }, idem });
      return dto;
    });
  ...
  async function updatePlan(command, context) {
    ...
    const result = await commitAtomically(ports, async (tx) => {
      ...
      const dto = planDto(saved);
      await recordSideEffects({ tx, ports, context, event: { result: dto }, audit: { action: 'planning.plan.updated', organizationId: context.organizationId, actorId: context.subjectId, targetId: dto.planId, correlationId: context.correlationId }, outbox: { type: 'planning.plan.updated', organizationId: context.organizationId, aggregateId: dto.planId, payload: dto, correlationId: context.correlationId }, idem });
      return dto;
    });
    return ok(result, metaFromContext(context));
  }
  async function readProfile(query, context) { const ctx = validateContext(context, 'getProfile'); if (ctx) return ctx; const scoped = await validateTenantScope({ ports, context, operation: NAMED_USE_CASES.getProfile.operationId }); if (scoped) return scoped; const found = await ports.persistencePort.readProfile({ organizationId: context.organizationId }); if (!found) return toFailed(REMOTE_PROBLEM_CODES.NOT_FOUND, PROBLEM_CATEGORIES.NOT_FOUND, { correlationId: context.correlationId }); return ok(profileDto(found), metaFromContext(context)); }
  async function replaceProfile(command, context) {
    const ctx = validateContext(context, 'upsertProfile'); if (ctx) return ctx;
    const scoped = await validateTenantScope({ ports, context, operation: NAMED_USE_CASES.upsertProfile.operationId }); if (scoped) return scoped;
