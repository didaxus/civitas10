...
function createOrganizationMappingRouter({ service }) {
  const router = express.Router({ mergeParams: true }); router.use(express.json({ limit: "128kb" }));
  router.post("/o/:organizationId/organization-model/drafts", requireDeclaredAction("organizationModel.editDraft"), async (req,res)=>{ try { res.status(201).json(await service.createDraft({ organizationId:req.params.organizationId, model:req.body?.model || {}, actorLogtoUserId:actor(req), idempotencyKey:req.get("Idempotency-Key") })); } catch(e) { sendError(res,e); } });
  router.put("/o/:organizationId/organization-model/drafts/:draftId", requireDeclaredAction("organizationModel.editDraft"), async (req,res)=>{ try { res.json(await service.updateDraft({ organizationId:req.params.organizationId, draftId:req.params.draftId, model:req.body?.model || {}, expectedVersion:req.get("If-Match") || req.body?.expectedVersion, actorLogtoUserId:actor(req), idempotencyKey:req.get("Idempotency-Key") })); } catch(e) { sendError(res,e); } });
  router.post("/o/:organizationId/organization-model/evaluations", requireDeclaredAction("organizationModel.evaluateMappingPolicies"), async (req,res)=>{ try { res.status(201).json(await service.evaluate({ organizationId:req.params.organizationId, draftId:req.body?.draftId, policy:req.body?.policy || { rules: [] }, sourceFacts:req.body?.sourceFacts || {}, sourceConnectionId:req.body?.sourceConnectionId, actorLogtoUserId:actor(req), idempotencyKey:req.get("Idempotency-Key") })); } catch(e) { sendError(res,e); } });
  router.post("/o/:organizationId/organization-model/drafts/:draftId/preview", requireDeclaredAction("organizationModel.evaluateMappingPolicies"), async (req,res)=>{ try { res.status(201).json(await service.preview({ organizationId:req.params.organizationId, draftId:req.params.draftId, actorLogtoUserId:actor(req), idempotencyKey:req.get("Idempotency-Key") })); } catch(e) { sendError(res,e); } });
  router.post("/o/:organizationId/organization-model/drafts/:draftId/publish", requireDeclaredAction("organizationModel.publishVersion"), async (req,res)=>{ try { res.status(201).json(await service.publish({ organizationId:req.params.organizationId, draftId:req.params.draftId, expectedDraftVersion:req.body?.expectedDraftVersion, previewId:req.body?.previewId, expectedPreviewDigest:req.body?.expectedPreviewDigest, reason:req.body?.reason, actorLogtoUserId:actor(req), idempotencyKey:req.get("Idempotency-Key") })); } catch(e) { sendError(res,e); } });
  router.post("/o/:organizationId/organization-model/publications/:publicationId/rollback-draft", requireDeclaredAction("organizationModel.createRollbackDraft"), async (req,res)=>{ try { res.status(201).json(await service.createRollbackDraft({ organizationId:req.params.organizationId, publicationId:req.params.publicationId, reason:req.body?.reason, actorLogtoUserId:actor(req), idempotencyKey:req.get("Idempotency-Key") })); } catch(e) { sendError(res,e); } });
  router.post("/o/:organizationId/organization-model/evaluations/:evaluationId/reviews", requireDeclaredAction("organizationModel.approveMapping"), async (req,res)=>{ try { res.status(201).json(await service.review({ organizationId:req.params.organizationId, evaluationId:req.params.evaluationId, decision:req.body?.decision, reason:req.body?.reason, actorLogtoUserId:actor(req), idempotencyKey:req.get("Idempotency-Key") })); } catch(e) { sendError(res,e); } });
  router.get("/o/:organizationId/organization-model/audit", requireDeclaredAction("organizationModel.inspectAuditHistory"), async (req,res)=>{ try { res.json(await service.listAudit({ organizationId:req.params.organizationId, limit:req.query.limit })); } catch(e) { sendError(res,e); } });
  return router;
}
