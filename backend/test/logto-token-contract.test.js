...
test('principal paths remain independently membership, role, tenant, and freshness bound', async () => {
  const claims = { iss: 'https://issuer.test/oidc', sub: 'subject_123', aud: 'https://civitas.didaxus.com/api', organization_id: 'org_123', scope: 'lms.groups.read', [claimsContract.MEMBERSHIP_CLAIM]: 'membership_123', [claimsContract.ROLES_CLAIM]: ['role_teacher', 'role_advisor'], [claimsContract.VERSION_CLAIM]: claimsContract.AUTHZ_CONTRACT_VERSION }
  const input = { claims, session: { organizationId: 'org_123', bindingVersion: 2, currentBindingVersion: 2 }, tenantContext: { organizationId: 'org_123', contextVersion: 3, currentContextVersion: 3 }, routeOrganizationId: 'org_123', resourceOrganizationId: 'org_123', requiredScope: 'lms.groups.read', providers: { getOrganization: async () => ({ status: 'active' }), getMembership: async () => ({ id: 'membership_123', organizationId: 'org_123', subject: 'subject_123', status: 'active', snapshotVersion: 4, currentSnapshotVersion: 4 }), getRoleBinding: async ({ logtoRoleId }) => ({ status: 'active', surface: 'organization', canonicalRoleId: logtoRoleId === 'role_teacher' ? 'organization_teacher' : 'organization_advisor' }) } }
  const principal = await buildOrganizationPrincipal(input)
  assert.equal(principal.paths.length, 2)
  assert.equal(principal.paths.every((path) => path.membershipId === 'membership_123' && path.tokenScopePresent), true)
