...
function fixture(overrides = {}) {
  ...
  const definition = { id: 'planning.plan.update', version: '1.0.0', status: 'active', exposure: 'curated', applicationServiceId: 'updatePlan', permission: 'planning.plans.manage', effect: 'write', risk: 'R2', confirmationPolicyVersion: 'policy/v1', requiresIfMatch: true, makerChecker: true, timeoutMs: 25,
  ...
  const request = { toolId: definition.id, input, tenantId: 'tenant-a', idempotencyKey: 'idem-1', fingerprint: fp, consentProof: 'signed-proof', confirmationPolicyVersion: 'policy/v1', confirmationNonce: 'nonce-1', ifMatch: '1', approvalId: 'approval-1' };
...
test('invokes an active application service directly and logs only security metadata', async () => {
  ...
  assert.equal(f.state.audits[0].critical, true); assert.deepEqual(f.state.audits[0].normalizedDiff, []);
});
...
test('rejects confirmation policy downgrade', async () => { const f = fixture(); await rejectsCode(f.gateway.execute({ ...f.request, confirmationPolicyVersion: 'policy/v0' }, f.context), TOOL_PROBLEMS.CONFIRMATION_POLICY); });
